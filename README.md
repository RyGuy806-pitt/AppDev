...
UPDATE: Activity issue was avoided.

This login page is relatively simple at the moment, and consists of two EditTexts boxs, capable of storing a word to a variable in the java code, that would ideally be checked against a data base once login is clicked. At the moment, if the user were to log out, there seems to be two issues that occur. The first is that upon trying to reload the login screen, the app seems to crash. I am not sure what the exact reason for this is, so I have not included a log out option, and would intend for the user to just have to log in each time, as tedious as that may seem. The other issue is that switching between multiple tabs too rapidly as well as going from the login to data page too quickly seems to make it so the charts cannot be loaded in properly.

![Screenshot from 2022-07-27 12-40-38](https://user-images.githubusercontent.com/77860961/181305039-004bc7f1-35a3-403f-b9d2-66c7a2a8bb35.png)
